import { pickColumns } from '@repo/shared/db/helpers';
import { and, desc, eq, getTableColumns, ne } from 'drizzle-orm';
import { alias } from 'drizzle-orm/pg-core';
import { type Db, db, type Tx } from '#/db/server.ts';
import type {
	InsertDmMember,
	InsertDmRoom,
...
type InsertChannel = typeof channelTable.$inferInsert;
export const _insertChannel = async (db: Db, data: InsertChannel[]) =>
...
export const _listDmRoom = async (authId: string) => {
	...
	const dmMembers = await db
		.select({
			...
			...pickColumns(userTable, userItemFields),
		})
		.from(dmMemberTable)
		.innerJoin(userTable, eq(userTable.id, dmMemberTable.user_id))
